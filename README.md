...
This repository generates first-order regular perturbation on gamma (FRP) approximate solution to the Manakov equation considering the following *scenarios*:
...
The main goal of this repostory is to create synthetic data based on these models. This repository is based on pre-computed kernels which are available only under certain conditions (see below).

## Quick start
1. Download or clone this repository
2. Run "Setup_Repo.m"
3. The repository is now ready to start simulating either of the three FRP scenarios enlisted above. For example, go to 3_Examples/NBGD/ and run GenerateFRP_NBGDoptimized.m

## Summary 
The repository contains the following folders:
- **1_HelperFunctions:** containing basic functions
- **2_Kernels:** containing the pre-calcualted FRP kernels of the three scenarios enlisted above.
...
**Disclaimer:** The parameter _P.Model.Nsam_ must be chosen carefully because of the memory required for the computations. Beware that the expression of the FRP model requires the computation of many triplets, which must be done for every symbol in your transmitted sequence. In its implementation, we generated all the triplets simultaneously, which generates an array of triplets of size $N_{\text{sam}}\times(2M+1)^3$, with $N_{\text{sam}}$ being the number of symbols in the transmitted sequence. 
